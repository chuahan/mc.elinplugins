...
///     When Corrupted: Immediate Return to bound home Location, but leaves a portal in the departure location, allowing
///     return.
...
public class ActGateway : Ability
{
    public override bool CanPerform()
    {
        ...
        if (_zone == pc.homeZone) return false;
    ...
    public override bool Perform()
    {
        ...
        if (isCorrupted)
        {
            ...
            if (gateway != null)
            {
                ...
                Zone checkZone = game.spatials.Find(gateway.DestinationUid);
                if (checkZone != null)
                {
                    player.returnInfo = new Player.ReturnInfo
                    {
                        turns = 1,
                        isEvac = true,
