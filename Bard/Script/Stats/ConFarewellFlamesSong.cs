...
public class ConFarewellFlamesSong : ConBardSong
{
    ...
    public override void OnRemoved()
    {
        if (BorrowedTime)
        {
            // If they remain in a critical state (20%) when the buff expires, they will just die.
            if (owner.hp < owner.MaxHP / 5)
            {
                Msg.Say("flamesong_fate".langGame(owner.NameSimple));
                HelperFunctions.DamageHpWrapper(owner, HelperFunctions.SafeMultiplier(owner.MaxHP, 5), Constants.EleVoid, 100, AttackSource.Finish, null);
            }
            else
            {
