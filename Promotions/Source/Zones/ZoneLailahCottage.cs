...
public class ZoneLailahCottage : Zone_Civilized
{
    ...
    {
        if (_map.charas.Find(c => c.id == Constants.LailahCharaId) != null)
        {
            return;
        }

        // If Lailah has joined the PC Faction.
        Chara? lailah = game.cards.globalCharas.Values.FirstOrDefault(gc => gc.id == Constants.LailahCharaId);
        ...
        if (lailah != null)
        {
            Room? targetRoom = map.rooms.listRoom.FirstOrDefault(r => r.Name.Equals("CottageMain", StringComparison.InvariantCultureIgnoreCase));
            Point lailahSpot = _map.GetCenterPos().GetNearestPoint(false, false);
            if (targetRoom != null)
            {
